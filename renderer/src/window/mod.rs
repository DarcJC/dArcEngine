...
use crate::component::{Action, GSCHEDULES, GWORLD};
...
lazy_static! {
    pub static ref APPLICATION_CONTEXT: RwLock<ApplicationContext<'static>> = RwLock::new(ApplicationContext::new());
}
...
pub struct ApplicationContext<'a> {
    ...
    display_component: Option<crate::component::DisplayComponent>,
...
impl<'a> ApplicationContext<'a> {
    pub fn new() -> ApplicationContext<'a> {
    ...
    pub async fn run(mut lock: RwLockWriteGuard<'static, Self>) {
        ...
        if lock.display_component.is_none() {
            lock.display_component = Some(crate::component::DisplayComponent::new(lock.window.as_mut().unwrap()).await);
        }
